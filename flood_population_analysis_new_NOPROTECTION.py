# filepath: c:\Users\pgwie\OneDrive - office365.boku.ac.at\UBRM\Bachelorarbeit\Analysis\flood_population_analysis_new.py
...
def main(population_file_path: str, income_file_path: str, flood_file_path: str, base_output_dir=None, 
    ...
    run_specific_output_dir = os.path.join(base_output_dir, specific_run_output_dir_name)
    ...
    log_file = setup_logging(run_specific_output_dir) 
    ...
    if pop_data is not None and flood_data is not None and income_data is not None and protection_data is not None and nuts_data_loaded is not None:
        ...
        # --- 3. Visualize data alignment (optional, for debugging) ---
        # Use a sample NUTS region for this, e.g. Vienna AT13 or a small country
        # This should use the *protected* flood map for the main analysis path
        # For the NOPROTECTION script, this should use the *original* flood_data
        output_dir_viz_align = os.path.join(run_specific_output_dir, "visualize_alignment_checks")
        os.makedirs(output_dir_viz_align, exist_ok=True)
        
        # For the NOPROTECTION script, we want to use 'flood_data' here
        visualize_data_alignment_with_nuts(
            pop_data, 
            flood_data, # MODIFIED: Was flood_data_protected
            income_data, 
            nuts_data_loaded, 
            output_dir=output_dir_viz_align
        )

        # Prepare final data for analysis function
        pop_data_for_analysis = pop_data.fillna(0)
        income_data_for_analysis = income_data # Assuming NaNs handled or accepted by analysis
        
        # For the NOPROTECTION script, this should use the original aligned flood data:
        flood_data_for_analysis = flood_data.fillna(0) # MODIFIED: Was flood_data_protected.fillna(0)
        
        print_memory_usage("After preparing final data for analysis")
        ...
        nuts_pop_by_depth_income = analyze_population_by_flood_depth_and_income(
            pop_data, flood_data_for_analysis, income_data=income_data, 
        ...
        if run_specific_output_dir and nuts_pop_by_depth_income is not None:
            print("Saving results to GeoJSON...")
            geojson_path = os.path.join(run_specific_output_dir, f'analysis_results_{ssp_name}_{year}_{flood_event_name}.geojson')
            ...
            csv_path = os.path.join(run_specific_output_dir, f'detailed_analysis_results_{ssp_name}_{year}_{flood_event_name}.csv')
        ...
        if nuts_pop_by_depth_income is not None:
            visualize_population_by_income_and_depth(
                nuts_pop_by_depth_income, run_specific_output_dir,
        ...
        print("\\n====== ANALYSIS COMPLETE ======")
        print(f"Results saved to {run_specific_output_dir}")
...
if __name__ == "__main__":
    ...
    base_output_directory = '/hdrive/all_users/wiederkehr/analysis/analysis_runs_output_all_combinations_NOPROTECTION' # MODIFIED
